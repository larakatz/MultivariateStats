...
\newline
*I will keep the first four PCA axes because they have eigenvalues > 1 (latent root criterion). Cumulatively, these four axes explain 93.8% of the variation in the data. After PCA4, the scree plot begins to level off.*
...
*PC1 is being driven primarily by population parameters (`manu` and `popul`) and temperature (`temp`).
PC2 is primarily being driven by precipitation parameters (`predays` and `precip`) and population parameters (`popul` and `manu`).*
...
*I cannot use PCA on my data because, even with variable transformations (below), they do not meet the assumption of multivariate normality. The majority of the data points are independent pseudo-absence points generated by `terra`, but the Bridle Shiner survey locations were sampled in person and most were not random. Taking a stratified random sample of the in-person survey locations would allow the data to meet the independent sample assumption. The data do meet some of the data requirements for PCA: There are more samples (n = 10,000) than there are columns (n = 38), which meets the rule of thumb that n > 3x38. The majority of the variables are continuous, three are categorical, and the response variable is binary (I removed the binary response variable when I standardized the data). Every sample entity is measured for the same set of variables and there are no NA values.* 
